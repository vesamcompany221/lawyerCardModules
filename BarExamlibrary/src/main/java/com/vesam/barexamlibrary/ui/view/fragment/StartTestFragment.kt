...
class StartTestFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentStartTestBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initOnClick() {
        binding.lnStartTest.setOnClickListener { navController.navigate(R.id.examFragment) }
    }
    ...
    private fun startAutoSlider(count: Int) {
        runnable = Runnable {
            ...
            handler.postDelayed(runnable, BuildConfig.DELAYED_SLIDER)
        }
        handler.postDelayed(runnable, BuildConfig.DELAYED_SLIDER)
    }
